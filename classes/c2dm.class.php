 ...
 * Service from google. The service is available for the Android 
...
class C2DM extends Main implements Mobile_Push
{
    ...
    **/
    ...
    private $api_gateway_developer      = 'https://android.apis.google.com/c2dm/send';
    private $api_gateway_production     = 'https://android.apis.google.com/c2dm/send';
    ...
    **/
     ...
     * @param string $auth_username The developers google account email address
    ...
    **/
    public function set_api_credentials($type, $auth_username, $auth_password, $auth_source)
    {
        parent::debug2(__FUNCTION__.' called with type='.$type.' , auth_username='.$auth_username.' , auth_password='.$auth_password.' , auth_source='.$auth_source);
        
        // check the type
        if($type === 'production' || $type === 'developer')
        {
            if(is_string($auth_username) === TRUE && is_string($auth_password) === TRUE)
            {
                ...
                $this->credentials_username = $auth_username;
                $this->credentials_password = $auth_password;
                $this->credentials_source   = $auth_source;
                parent::debug('Authenticate credentials: email='.$auth_username);
    ...
    **/
    public function get_api_credentials($type = 'production')
    {
        ...
        return array('username' => $this->credentials_username, 'password' => $this->credentials_password, 'source' => $this->credentials_source);
    }
    ...
    **/
    private function _build_message_payload()
    {
        ...
        $payload = 'registration_id='.urlencode($this->message_content['device_token']).'&collapse_key=new';
    ...
    **/
     ...
     * C2DM gateway, will NOT arrive on the destination device, and may cause
    ...
    **/
    public function send_push_message()
    {
        ...
        $push_header = array('Content-type: application/x-www-form-urlencoded;charset=UTF-8', 'Content-Length: '.$push_post_length, 'Authorization:GoogleLogin auth='.$this->service_auth_token);
        ...
        if($curl_info['http_code'] !== 200)
        {
            ...
            {
                parent::info('Returned HTTP code 503 / c2dm reports that resource is temporarily unavailable');
            }
            elseif($curl_info['http_code'] == 401)
            {
                parent::debug('Returned HTTP code 401 / c2dm reported that the ClientAuth sent with the request no longer valid');
        ...
        // see http://code.google.com/android/c2dm/#received_data for possible answers
        $matches = array();
